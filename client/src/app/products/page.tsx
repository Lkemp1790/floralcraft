  ...
  const filtered = useMemo(() => {
    return productDummyData
      .filter((p) => {
        const byType =
          types.length === 0 ||
          types.some((t: string) => p.categories.includes(t));
        const byFlower =
          flowers.length === 0 ||
          flowers.some((f: string) => p.flowerTypes.includes(f));
        const byPrice =
          price === "any" ||
    ...
    <main>
      ...
      <section className="w-full py-10 px-4 sm:px-6 lg:px-8">
        ...
        <div className="max-w-7xl mx-auto">
              ...
              <div className="rounded-xl border border-[#0D383B]/10 bg-white shadow-sm overflow-hidden">
                ...
                <div className="p-5 grid gap-3">
                  {ALL_COLLECTIONS.map((c) => (
                    <label
                      key={c}
                      className="flex items-center gap-3 text-sm text-[#0D383B]"
                    >
                      <input
                        type="checkbox"
                        checked={collections.includes(c)}
                        onChange={() =>
                          toggleIn(collections, c, setCollections)
                        }
          ...
          <div
            id="filters-drawer"
            className="lg:hidden fixed inset-x-0 bottom-0 z-50 translate-y-full transition-transform duration-300"
          >
            <div className="bg-white rounded-t-2xl shadow-2xl border border-[#0D383B]/10 max-h-[80vh] overflow-y-auto">
              ...
              <div className="p-4 space-y-6">
                ...
                <div className="rounded-xl border border-[#0D383B]/10 bg-white overflow-hidden">
                  <div className="px-4 py-3 border-b border-[#0D383B]/10">
                    <h4 className="font-serif text-[#0D383B]">Collections</h4>
                  </div>
                ...
                <div className="rounded-xl border border-[#0D383B]/10 bg-white overflow-hidden">
                  <div className="px-4 py-3 border-b border-[#0D383B]/10">
                    <h4 className="font-serif text-[#0D383B]">Price</h4>
                  </div>
                ...
                <div className="rounded-xl border border-[#0D383B]/10 bg-white overflow-hidden">
                  <div className="px-4 py-3 border-b border-[#0D383B]/10">
                    <h4 className="font-serif text-[#0D383B]">Type</h4>
                  </div>
                  <div className="p-4 grid gap-3">
                    {ALL_TYPES.map((t) => (
                      <label
                ...
                <div className="rounded-xl border border-[#0D383B]/10 bg-white overflow-hidden">
                  <div className="px-4 py-3 border-b border-[#0D383B]/10">
                    <h4 className="font-serif text-[#0D383B]">Flowers</h4>
                  </div>
                  <div className="p-4 grid gap-3">
                    {ALL_FLOWERS.map((f) => (
                      <label
